...
function ajaxFUN(type,id) {
    $.ajax({
        url: context + "/service/"+type+"/shenheid",
        type: 'GET',
        async: false,
...
function rentConfirm() {
    ...
    var carBrand = $(".car-brand").text();//问题类型
    var carName = $(".car-name").text();//商家名称
    var orderid = $(".car-id").text();
    var price = $(".price").text();
    if(user){
        if(price>0){
            var d = dialog({
                title:"罚款结算信息",
                content: '您因为'+carBrand +'问题'+'需要支付'+carName+'商家' +
                    '共'+price+'元罚款！',
                okValue: '结算',
                ok: function () {
                    window.location.href = context +"/jsp/index.jsp";
...
function rent(orderid) {
    var userId=getCookie("userId");
    var shopId=$(".shopId").attr("value");
    $.ajax({
        url: context + "/service/order/updateshenhe",
        type: 'GET',
        async: true,
        contentType: "application/json;charset=utf-8",
