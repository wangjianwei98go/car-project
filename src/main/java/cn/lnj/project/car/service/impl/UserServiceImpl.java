...
public class UserServiceImpl implements UserService {
    ...
    public JSONObject selectById(String userId) {
        ...
        if (orderList.size() > 0) {
            ...
            for (Order content : orderList) {
                ...
                order.put("state", content.getState());
        ...
        if (1 == user.getIsAdmin()) {
            List<Order> orderList1 = orderDao.selectByShopId(String.valueOf(user.getIsAdmin()));
            if (orderList1.size() > 0) {
                JSONArray orders1 = new JSONArray();
                for (Order content : orderList1) {
                    JSONObject order = new JSONObject();
                    //还需要根据车辆id和商家id查询出车辆名称，商家名称等信息
                    order.put("carId", content.getCarId());
                    Car car = carDao.selectById(content.getCarId());
                    ...
                    order.put("shopId", content.getShopId());
                    Shop shop = shopDao.selectById(content.getShopId());
                    User user1 = userDao.selectById(content.getUserId());
                    order.put("shopName", user1.getUserEmail());
                    order.put("username", user1.getUserName());
                    order.put("orderTime", content.getOrderTime());
                    order.put("endTime", content.getEndTime());
                    order.put("orderId", content.getOrderId());
                    order.put("state", content.getState());
                    if (content.getState() == 1) {
                        order.put("admintype", "租赁中");
                    }
                    if (content.getState() == 2) {
                        order.put("admintype", "审核");
                    }
                    if (content.getState() == 3) {
                        order.put("admintype", "审核详情");
                    }

                    orders1.add(order);
                }
                result.put("adminorders", orders1);
            }
    ...
    @Override
    public JSONObject checkCarId(String name, String param) {
        JSONObject jsonObject = new JSONObject();
        String result = "y";
        //登录时账号查询 存在且激活 通过
        if ("userCount".equals(name)) {
           carId user = carIdDao.selectBycarid(param);
            if (user != null) {
                result = "y";
            }
            if ("y".equals(result)) {
